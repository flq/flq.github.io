...
slug: json-and-local-storage-with-elm
---
...
Let us start with the user interaction. [The user clicks][2] on "Save current settings"...

```
a [onClick SaveAppState] [text "Save current settings"]
```

The message is handled in the __update__ function [by creating a Command][3] which will do the job. Let us look at the [contents of the command][4].
...
Let's go in reverse this time, beginning with the [retrieval functionality][7] implemented in javascript.
...
Let us look at the [subscription][8].
...
Using [the decoder][9] `modelDecoder`, it is attempted to retrieve the data from local storage. The decoder
